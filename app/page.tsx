...
export default function CinematicLandingPage() {
  ...
  const [backgroundElements, setBackgroundElements] = useState<{
    lines: { x1: number; y1: number; x2: number; y2: number; duration: number; delay: number }[];
    docs: { left: number; top: number; duration: number }[];
    nodes: { left: number; top: number; duration: number; delay: number }[];
  }>({ lines: [], docs: [], nodes: [] })

  useEffect(() => {
    // Generate random values only on client side to avoid hydration mismatch
    setBackgroundElements({
      lines: Array.from({ length: 20 }).map(() => ({
        x1: Math.random() * 1920,
        y1: Math.random() * 1080,
        x2: Math.random() * 1920,
        y2: Math.random() * 1080,
      ...
      docs: Array.from({ length: 4 }).map((_, i) => ({
        left: 15 + (i * 20),
        top: 25 + (i % 2) * 30,
        duration: 6 + i * 1
      })),
      nodes: Array.from({ length: 6 }).map(() => ({
    ...
    <div className="min-h-screen bg-black text-white overflow-hidden relative">
      ...
      <div className="fixed inset-0 z-0">
          ...
          <svg className="w-full h-full" viewBox="0 0 1920 1080">
            ...
            {backgroundElements.lines.map((line, i) => (
              <motion.line
                ...
                transition={{
                  duration: line.duration, // Slower animations
                  repeat: Infinity,
                  repeatType: "reverse",
                  delay: line.delay
                }}
        ...
        >
          ...
          {backgroundElements.docs.map((doc, i) => (
            <motion.div
              key={`doc-${i}`}
              ...
              transition={{
                duration: doc.duration, // Slower animations
                repeat: Infinity,
                ease: "easeInOut"
              }}
            >
          ...
          {backgroundElements.nodes.map((node, i) => (
            <motion.div
              ...
              transition={{
                duration: node.duration, // Slower animations
                repeat: Infinity,
                delay: node.delay
              }}
            />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <nav className="hidden md:flex space-x-8 items-center">
              <Link href="#" className="text-gray-300 hover:text-cyan-400 transition-colors duration-300">Home</Link>
              <Link href="#features" className="text-gray-300 hover:text-cyan-400 transition-colors duration-300">Features</Link>
              <Link href="/login" className="text-gray-300 hover:text-cyan-400 transition-colors duration-300">Login</Link>
              <Link href="/signup" className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-lg text-white font-medium hover:from-cyan-400 hover:to-purple-500 transition-all duration-300 shadow-lg shadow-cyan-500/25">
                Sign Up
              </Link>
            </nav>
